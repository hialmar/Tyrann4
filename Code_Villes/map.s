...
main_loop
	...
	jsr aff_hero			; affiche le hero au centre ... 
	jsr	aff_text
	ldy $17
...
; puis déterminons la position perso dans la carte
		...
		clc
		adc $06					; rang tuile ds ligne table dataMap en haut gauche fenêtre
		sta $08
...
sens_1
		...
		bne sens_2
		lda $08;dec $08					; $08 contient rang perso dans ligne map
		beq no_scroll ;bmi no_scroll			; le perso était en bord gauche map
		dec $08							; supprimer si retour	
...
sens_3
		...
		bne sens_4
		lda $07  ;dec $07
		beq no_scroll ;bmi no_scroll
		dec $07  ; a supprimer si retour
		jsr rech_tab_map		; en sortie  repère tuile dans $0a
		lda $0a
		beq check_ville_bateau			
		bne around_sortie			
...
sortie_scroll_direct
		rts
check_ville_bateau			; la tuile adjacente est la mer
		lda $0e				; valeur tuile à la position du perso 				
		beq around_sortie	; si on est déjà en mer 
		cmp #$01			
		beq around_sortie		
		cmp #$5e			; si on n'est pas déjà en mer , est-on dans un port ? Villes portuaires codées entre #$5e et #$63
...
init_div_var
	...
	lda #$01
	sta $0d			; drapeau bateau : 1 on a un bateau / 0 pas de bateau	(à 1 provisoire pour est map)
	sta $1c			; indique lieu <> Gallia (0)		
	lda #$20
	sta $50			; drapeau sortie victorieuse de la carte = $80 sinon = $20
	rts
...
aff__tuile
			lda $12
			cmp $09					; n'affiche pas la tuile si c'est celle qui est sous le perso
...
;		beq end_key		;sur ecran 'Episode II' (sera à supprimer)
;		lda $208
;		cmp #$38
;		beq wait_key
;		cmp #$ac
...
;sortie vers ecran HIRES ;-)
	lda #$80
	sta $50				;drapeau mur atteint
	lda #$86
	sta $0c				; simule appui sur 'Y' pour sortie carte
	rts
;-----------------------------------------------------	
druid_hut	
	lda $0e
...
test_nord	
	lda $05
	cmp #$10				;et tenter de passer au nord de la ligne 19 de la carte
	bne fin_txt   
...
test_britania			; au nord de #$11 si on n'est ni en Germania, ni en Hibernia, ni en Caledonia
	lda $07				; alors on est soit en Britania, soit dans le Pas de Calais
	cmp #$0f
	bmi test_brit_2
	lda $08
	cmp #$15
