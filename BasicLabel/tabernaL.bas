...
ville_40
 RETURN
ville_7
 S$=" TABERNA ":MENU=0:N=6:ENC=EE(VIL):GOSUB ville_51 
 AL$="ALREADY EQUIPPED !"
...
Recruit_Elancia3
 ENC=2:S$="Recruit Elancia ? ":L=16:CLS:GOSUB ville_4 
 PLOT5,13,"DO YOU WANT TO RECRUIT ME (Y/N) ?"
Recruit_Elancia4
 GETA$
 IFA$="N"THEN ville_7 
 IFA$="Y"THEN GOSUB Recruit_Elancia5 
 GOTO ville_7 
Recruit_Elancia5
 GOSUB ville_5
 PLOT5,26,"REPLACE WHOM ?"
...
ville_sleep
 ...
 PLOT5,13,"DO YOU WANT TO SLEEP (Y/N) ?"
ville_sleep1
 GETA$
 IFA$="N"THEN ville_7 
 IFA$="Y"THEN GOSUB ville_sleep2 
 GOTO ville_7 
ville_sleep2
 FORP=1TO6
    IFHO<=RI(P)THEN ville_sleep3
 NEXT   
 S$="Seven gods, you're too poor !": RETURN 
ville_sleep3 
 RI(P)=RI(P)-HO
 CLS:PRINT @10,8;" A NICE NAP "
 GOSUB ville_recup:PRINT @7,10;" YOU RECOVER YOUR SPELLS":WAIT TI*5
 RETURN
ville_recup 
  FOR P=1TO6
    IF ET(P)>0 AND ET(P)<5 THEN ET(P)=ET(P)+FNA(3)
    IF CP(P)<4 THEN ville_recup2
     FOR I=1TONI(P)
       SN(P,I)=FNA(2)+2
     NEXT I
ville_recup2     
...
ville_LOAD
 ...
 REM FOR V=1TO9:FOR C=1TO4:O1=O1+1:CL(V,C)=PEEK(O1):NEXT C,V
 FOR V=1TO16:FOR C=1TO2:O1=O1+1:CL(V,C)=PEEK(O1):NEXT C,V
 FOR V=1TO4:O1=O1+1:TVDO(V)=PEEK(O1):NEXT V
 FOR I=1TO6:O1=O1+1:IG(I)=PEEK(O1):NEXT
 FOR V=1TO16:FORM=1TO2:O1=O1+1:TC(V,M)=PEEK(O1):NEXT M,V
 FOR V=1TO13:O1=O1+1:T4INF(V)=PEEK(O1):NEXT V
...
ville_SAVE
 ...
 REM FOR V=1TO9:FOR C=1TO4:O1=O1+1:POKEO1,CL(V,C):NEXT C,V
 FOR V=1TO16:FOR C=1TO2:O1=O1+1:POKEO1,CL(V,C):NEXT C,V
 FOR V=1TO4:O1=O1+1:POKEO1,TVDO(V):NEXT V
 FORI=1TO6:O1=O1+1:POKEO1,IG(I):NEXT
 REM FOR V=1TO9:FORM=1TO5:O1=O1+1:POKEO1,TC(V,M):NEXT M,V
 FOR V=1TO16:FORM=1TO2:O1=O1+1:POKEO1,TC(V,M):NEXT M,V
 FOR V=1TO13:O1=O1+1:POKEO1,T4INF(V):NEXT V
 O1=O1+1:POKEO1,DE
...
ville_DAT
 TI=20:HV=6:DF=0
 DIM IT$(55),PR%(55),IP$(10,2),TP$(2,15),TC(16,2),T4INF(13),CL(16,2)
 GOSUB  ville_99
...
ville_100
 ...
 DATA ARMORY,15,7, BOTANICS,7,2, TABERNA,5,3, ANIMALS,7,7
