#labels
 TEXT:CLS:PAPER 0:INK3
 POKE48035,0:POKE#26A,PEEK(#26A)AND254
 S$="Characters Editor":GOSUB  editor_0 :PRINT@8,4;S$
 GOSUB  editor_1 
editor_5
 REM EQUIPE
 L1$=" * TYRANN 4 - TEAM *  "
 L2$="CHARACTER   CULTURE    CAREER   LEV"
 L3$="Money       ML RC ST DX IG MS HP  "
 CLS:PRINT@8,1;CHR$(145)CHR$(128)+L1$+CHR$(144)
 L=3:PRINT@3,L;CHR$(145)CHR$(128)+L2$+CHR$(144)
 PRINT@3,L+1;CHR$(148)CHR$(128)+L3$+CHR$(144)
 L=5:TE=0
 FORI=1TO6
 IFCP(I)=1THENENC=131
 IFCP(I)=2THENENC=135
 IFCP(I)=3THENENC=134
 IFCP(I)=4THENENC=133
 IFCP(I)=5THENENC=132
 IFCP(I)=6THENENC=130
 PRINT@1,L;CHR$(ENC);I;N$(I);
 PRINT@17,L;M$(MP(I)):PRINT@27,L;C$(CP(I))
 PRINT@37,L;NI(I)
 PRINT@4,L+1;STR$(RI(I));" s."
 S1$=STR$(CC(I))+STR$(CT(I))+STR$(FO(I))+STR$(AG(I))
 S2$=STR$(IN(I))+STR$(FM(I))+STR$(PV(I))
 S$=S1$+S2$
 PRINT@16,L+1;S$:L=L+3:TE=TE+RI(I)
 NEXTI:PRINT@3,L-1;CHR$(148);CHR$(128)L3$CHR$(144)
 EN=4
 S$="CHOOSE HERO TO EDIT (1-6) ?":GOSUB  editor_2 :PRINT@5,23;S$
 S$="Save Changes: 0 or Q)uit":GOSUB  editor_0 :PRINT@6,24;S$
 GOSUB  editor_3 :IF P=0THEN GOSUB editor_4 :GOTO editor_5 
editor_17
 CLS:L=23:GOSUB  editor_6 'menu edition
 PRINT@6,3;"Fst Name:";N$(P)
 PRINT@6,5;"Culture: ";M$(MP(P))
 PRINT@6,7;"Career:";C$(CP(P))
 PRINT@6,9;"Level:";NI(P)
 PRINT@6,11;"XP:";XP(P)
 PRINT@6,13;"Hit Pts:";PV(P)
 PRINT@6,14;"Purse:";RI(P);"s."
 PRINT@6,15;"Melee Combat   :";CC(P)
 PRINT@6,16;"Ranged Combat  :";CT(P)
 PRINT@6,17;"Strength ......:";FO(P)
 PRINT@6,18;"Dexterity .....:";AG(P)
 PRINT@6,19;"Intelligence ..:";IN(P)
 PRINT@6,20;"Mental Strength:";FM(P)
 S$="Arrows or Q)uit":GOSUB editor_0 :PRINT@4,22;S$
 S$="M)odif":GOSUB editor_0 :MO$=S$
 S$="<- +>":GOSUB editor_0 :FLECHE$=S$
 REM EDITION
 X=26:LI=3:UP=0:L=1
editor_18
 REPEAT
editor_9
 PRINT@4,LI;"=>":IFLI<8THENPRINT@X,LI;MO$ELSEPRINT@X,LI;FL$
editor_7
 GETA$
 IFA$<>"M"ANDA$<>"Q"ANDASC(A$)<8ANDASC(A$)>11THEN editor_7 
 IF(A$="M"ANDLI>7)OR((ASC(A$)=8ORASC(A$)=9)ANDLI<8)THEN editor_7 
 PRINT@4,LI;"  ":PRINT@X,LI;"         "
 IFASC(A$)=10THENUP=0:GOSUB editor_8 :GOTO editor_9 
 IFASC(A$)=11THENUP=1:GOSUB editor_8 :GOTO editor_9 
 UNTIL (LI<8ANDA$="M")ORA$="Q"OR(LI>7AND(ASC(A$)=8ORASC(A$)=9))
 IFA$="Q"THEN editor_5 
 PRINT@4,LI;"=>":IFLI<4THENPRINT@X,LI;MO$ELSEPRINT@X,LI;FL$
 IFLI<15 THEN ON L GOSUB  editor_10 , editor_11 , editor_12 , editor_13 , editor_XP, editor_14 , editor_15 
 IFLI>14 THEN GOSUB  editor_16 
 IFL<4THEN editor_17 ELSE editor_18 
editor_0
 REM FOND BLEU
 S$=" "+CHR$(148)+CHR$(131)+S$+CHR$(131)+CHR$(144)+" "
 RETURN
editor_2
 REM FOND ROUGE
 S$=" "+CHR$(145)+CHR$(135)+S$+CHR$(131)+CHR$(144)
 RETURN
editor_3
 GETA$:IF A$="Q" OR A$="q" THEN END
 P=VAL(A$):IFP>6THENGOTO  editor_3 
 RETURN
editor_19
 S$="< SPACE > ":GOSUB editor_2 
 PRINT@13,L;S$:GETA$:IF A$<>" "THEN editor_19 
 RETURN
editor_8
 REM descend ou monte
 IFUP=1THEN  editor_20 
 L=L+1
 IFLI<13THENLI=LI+2ELSELI=LI+1
 IFL=14THENLI=3:L=1
 GOTO editor_21 
editor_20
 L=L-1
 IF LI>13THENLI=LI-1ELSELI=LI-2
 IF L=0THENLI=20:L=13
editor_21
 RETURN
editor_10
 PRINT@11,4;"=>";:INPUTN$(P)
 IFLEN(N$(P))<2THENZAP:GOTO  editor_10 
 IFLEN(N$(P))>10THENN$(P)=LEFT$(N$(P),10)
 NN$=N$(P):N$(P)=LEFT$(NN$,1)
 FORI=2TOLEN(NN$)
 MI=ASC(MID$(NN$,I,1))
 IF MI>64 AND MI<91 THEN MI=MI+32
 L$=CHR$(MI)
 N$(P)=N$(P)+L$
 NEXT
 PRINT@22,4;"            "
 RETURN
editor_11
 REM HOUSES
 FORI=1TO9
 PRINT@25,4+I;I;M$(I):NEXT
editor_22
 GETM$:M=VAL(M$)
 IFM<1ORM>9THENGOTO editor_22 
 MP(P)=M
 RETURN
editor_12
 REM CARRIERE
 FORI=1TO6
 PRINT@25,6+I;I;C$(I):NEXT
editor_23
 GETM$:C=VAL(M$)
 IFC<1ORC>6THENGOTO editor_23 
 CP(P)=C
 RETURN
editor_13
 REM LEVEL
 CPT=NI(P):MAX=8:MIN=1:BO=1:GOSUB  editor_24 
 NI(P)=CPT
 PRINT@12,9;NI(P)
 RETURN
editor_XP
 REM XP
 CPT=XP(P):MAX=10000:MIN=1:BO=1000:GOSUB  editor_24 
 XP(P)=CPT
 PRINT@9,11;XP(P)
 RETURN
editor_14
 REM HP
 CPT=PV(P):MAX=99:MIN=1:BO=1:GOSUB  editor_24 
 PV(P)=CPT
 PRINT@14,13;PV(P)
 ET(P)=CPT
 RETURN
editor_15
 REM MONEY
 CPT=RI(P):MAX=60000:MIN=0:BO=1000:GOSUB  editor_24 
 RI(P)=CPT
 PRINT@12,14;RI(P)"s. "
 RETURN
editor_16
 REM CARACS
 ON LI-14 GOTO  editor_25 , editor_26 , editor_27 , editor_28 , editor_29 , editor_30 
editor_25
 CPT=CC(P):GOTO editor_31 
editor_26
 CPT=CT(P):GOTO editor_31 
editor_27
 CPT=FO(P):GOTO editor_31 
editor_28
 CPT=AG(P):GOTO editor_31 
editor_29
 CPT=IN(P):GOTO editor_31 
editor_30
 CPT=FM(P)
editor_31
 MAX=99:MIN=1:BO=1:GOSUB  editor_24 
 ON LI-14 GOTO  editor_32 , editor_33 , editor_34 , editor_35 , editor_36 , editor_37 
editor_32
 CC(P)=CPT:GOTO editor_38 
editor_33
 CT(P)=CPT:GOTO editor_38 
editor_34
 FO(P)=CPT:GOTO editor_38 
editor_35
 AG(P)=CPT:GOTO editor_38 
editor_36
 IN(P)=CPT:GOTO editor_38 
editor_37
 FM(P)=CPT
editor_38
 PRINT@22,LI;CPT
 
 RETURN
editor_24
 REM incrementeur
 IFASC(A$)=10ORASC(A$)=11THENRETURN
 IFASC(A$)=9THEN CPT=CPT+BO ELSE CPT=CPT-BO
 IF CPT > MAX THEN CPT=MAX
 IF CPT < MIN THEN CPT=MIN
 RETURN
editor_6
 CLS:PRINT
 PRINT" ******************
 FORI=1TOL:PRINT@2,I;"*":PRINT@38,I;"*":NEXTI
 PRINT@2,I;"*************
 RETURN
editor_4
 REM SAUVEGARDE
 S$="SURE YOU WANT TO SAVE CHANGES ?":GOSUB editor_2 
 PRINT@3,23;S$
 S$="             (Y/N)             ":GOSUB editor_2 
 PRINT@3,24;S$
editor_39
 A$=KEY$:IFA$=""THEN editor_39 
 IFA$<>"Y"THEN  editor_5 
 ZAP:GOSUB  editor_40 
 RETURN
editor_1
 FORI=1TO6:READ C$(I):NEXTI
 FORI=1TO9:READ M$(I):NEXTI:
 DIM CB(20),IT$(48)
 'RECALL IT$,"T-ITEMS",S
 PRINT@5,24;"INSERT DISK IN DRIVE"
editor_41
 PRINT@5,25;"  THEN PRESS SPACE"
 IFKEY$<>" "THENGOTO editor_41 
 GOSUB editor_42 
 LOAD"TEAM.BIN"
 O1=#A000
 O1=O1+1:VER=PEEK(O1)
 REM PRINT"Version " VIL
 O1=O1+1:XX=PEEK(O1)
 O1=O1+1:Y=PEEK(O1)
 O1=O1+1:S=PEEK(O1)
 O1=O1+1:CA=PEEK(O1)
 O1=O1+1:VIL=PEEK(O1)
 FOR P=1TO6
 O1=O1+1:DD=PEEK(O1)
 FORJ=1TODD:O1=O1+1:N$(P)=N$(P)+CHR$(PEEK(O1)):NEXTJ
 O1=O1+1:RI(P)=DEEK(O1)*10:O1=O1+2:CP(P)=PEEK(O1)
 O1=O1+1:MP(P)=PEEK(O1)
 O1=O1+1:CC(P)=PEEK(O1)
 O1=O1+1:CT(P)=PEEK(O1)
 O1=O1+1:FO(P)=PEEK(O1)
 O1=O1+1:AG(P)=PEEK(O1)
 O1=O1+1:IN(P)=PEEK(O1)
 O1=O1+1:FM(P)=PEEK(O1)
 O1=O1+1:PV(P)=PEEK(O1):GOSUB editor_43 
 O1=O1+1:ET(P)=PEEK(O1)
 O1=O1+1:OK(P)=PEEK(O1)
 O1=O1+1:NI(P)=PEEK(O1)
 O1=O1+1:XP(P)=DEEK(O1)
 O1=O1+2:WR(P)=PEEK(O1)
 O1=O1+1:WL(P)=PEEK(O1)
 O1=O1+1:PT(P)=PEEK(O1)
 O1=O1+1:CA(P)=PEEK(O1)
 O1=O1+1:BT(P)=PEEK(O1)
 FORI=1TO6:O1=O1+1:SAD(P,I)=PEEK(O1):NEXTI
 IF CP(P)>3 THEN FORI=1TO8:O1=O1+1:SN(P,I)=PEEK(O1):NEXT
 GOSUB editor_43 :NEXT P
 O1=O1+1:BS=PEEK(O1)
 O1=O1+1:FI=PEEK(O1)
 O1=O1+1:SD=PEEK(O1):GOSUB editor_43 
 FOR V=1TO9:FOR C=1TO4:O1=O1+1:CL(V,C)=PEEK(O1):NEXT C,V
 FOR I=1TO6:O1=O1+1:IG(I)=PEEK(O1):NEXT
 FOR V=1TO9:FORM=1TO5:O1=O1+1:TC(V,M)=PEEK(O1):NEXT M,V
 O1=O1+1:DE=PEEK(O1):GOSUB editor_43 
 O1=O1+1:TL=PEEK(O1):'PRINT "TL";TL:REM FR = FRE("")
 O1=O1+1:NP=PEEK(O1)
 O1=O1+1:NF=PEEK(O1)
 O1=O1+1:PM=PEEK(O1)
 IF VER>1 THEN O1=O1+1:OUT=PEEK(O1)
 GOSUB editor_43
 GOSUB editor_43 
 RETURN
editor_40
 TEXT:CLS
 PRINT@8,2;CHR$(145);CHR$(135);"+ TEAM PREP + ";CHR$(144)
 O1=#A000::GOSUB editor_44 
 POKEO1,0
 O1=O1+1:POKEO1,2
 O1=O1+1:POKEO1,XX
 O1=O1+1:POKEO1,Y
 O1=O1+1:POKEO1,S
 O1=O1+1:POKEO1,CA
 O1=O1+1:POKEO1,VIL
 FOR P=1TO6
 O1=O1+1:POKEO1,LEN(N$(P))
 FORJ=1TOLEN(N$(P)):O1=O1+1:POKEO1,ASC(MID$(N$(P),J,1)):NEXT
 O1=O1+1:DOKEO1,INT(RI(P)/10)
 O1=O1+2:POKEO1,CP(P)
 O1=O1+1:POKEO1,MP(P)
 O1=O1+1:POKEO1,CC(P)
 O1=O1+1:POKEO1,CT(P)
 O1=O1+1:POKEO1,FO(P)
 O1=O1+1:POKEO1,AG(P)
 O1=O1+1:POKEO1,IN(P)
 O1=O1+1:POKEO1,FM(P)::GOSUB editor_43 
 O1=O1+1:POKEO1,PV(P)
 O1=O1+1:POKEO1,ET(P)
 O1=O1+1:POKEO1,OK(P)
 O1=O1+1:POKEO1,NI(P)
 O1=O1+1:DOKEO1,XP(P)
 O1=O1+2:POKEO1,WR(P)
 O1=O1+1:POKEO1,WL(P)
 O1=O1+1:POKEO1,PT(P)
 O1=O1+1:POKEO1,CA(P)
 O1=O1+1:POKEO1,BT(P)::GOSUB editor_43 
 FORI=1TO6:O1=O1+1:POKEO1,SAD(P,I):NEXTI
 IF CP(P)>3 THEN FORI=1TO8:O1=O1+1:POKEO1,SN(P,I):NEXT
 NEXT P
 O1=O1+1:POKEO1,BS
 O1=O1+1:POKEO1,FI
 O1=O1+1:POKEO1,SD:GOSUB editor_43 
 FOR V=1TO9:FOR C=1TO4:O1=O1+1:POKEO1,CL(V,C):NEXT C,V
 FORI=1TO6:O1=O1+1:POKEO1,IG(I):NEXT
 FOR V=1TO9:FORM=1TO5:O1=O1+1:POKEO1,TC(V,M):NEXT M,V
 O1=O1+1:POKEO1,DE
 O1=O1+1:POKEO1,TL:REM PRINT "TL";TL
 O1=O1+1:POKEO1,NP:GOSUB editor_43 
 O1=O1+1:POKEO1,NF
 O1=O1+1:POKEO1,PM
 O1=O1+1:POKEO1,OUT:GOSUB editor_43 
 :SAVEU "TEAM.BIN",A#A000,EO1:REM FR = FRE("")
 REM SAVEU "TEAM2.BIN",A#A000,EO1
 RETURN
editor_42
 CLS:PRINT@7,8;"<<< Loading <<< Wait <<<"
 S$=CHR$(148)+" "+CHR$(144):CU=1:GOTO editor_43 
editor_44
 PRINT@7,8;">>> Saving >>> Wait  >>>"
 S$=CHR$(145)+" "+CHR$(144):CU=1
editor_43
 CU=CU+2:PRINT@CU,9;S$:REM FR = FRE("")
 RETURN
 DATA Legionary, Gladiator, Scout, Druid, Priest, Vestal
 DATA Celtic, Egyptian, Gallic, Goth, Persian, Roman, Viking, Iberian, Thrace
 