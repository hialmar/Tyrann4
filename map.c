...
void init_map() {
    ...
    //  et la fonction my_rnd() elle-même attend un uchar en parametre !!!)
    n = my_rnd(200)+51; // ne marche pas si #include <lib.h>
    //n = 230;
    printf("Ajout de %d arbres...\n", n);
    for(k = 0; k < n; k++) {
        kx = my_rnd(MAP_YSIZE-3)+1;
        ky = my_rnd(MAP_XSIZE-3)+1;
   ...
   n = my_rnd(60)+41; // ne marche pas si #include <lib.h>
   //n = 71;
    printf("Ajout de %d montagnes...\n", n);
    for(k = 0; k < n; k++) {
        kx = my_rnd(MAP_XSIZE-5)+1;
        ky = my_rnd(MAP_YSIZE-2)+1;
        //map[ky][kx++] = HILL1;
        //map[ky][kx]   = HILL2;
    ...
    n = my_rnd(30)+21; // ne marche pas si #include <lib.h>
    //n = 41;
    printf("Ajout de %d lacs...\n", n);
    for(k = 0; k < n; k++) {
        kx = my_rnd(MAP_XSIZE-8)+1;
        ky = my_rnd(MAP_YSIZE-5)+1;
