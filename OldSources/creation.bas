...
429 NEXT
430 PRINT:S$="PRENOM DU PERSO No"+STR$(P)+" (10 lettres max)":GOSUB 1950
435 INPUTN$(P)
...
627 PRINT@2,4;"FORMULE CARAC:";:PRINT@24,4;"Bonus"
628 PRINT@2,5;"Base 15    +    2D  +";:PRINT@24,5;"Carriere"
630 S$="CC:"+CAR$(1)+">":  GOSUB1900:GOSUB19000:GOSUB 20010:CC(P)=D1+D2+15+BC(CP(P),1)
635 S$=STR$(BC(CP(P),1))+" >":PRINT@26, 7;S$;:S$=STR$(CC(P))+"  ":GOSUB1950:PRINT
640 S$="CT:"+CAR$(2)+" > ":GOSUB1900:GOSUB19000:GOSUB 20010:CT(P)=D1+D2+15+BC(CP(P),2)
645 S$=STR$(BC(CP(P),2))+" >":PRINT@26,10;S$;:S$=STR$(CT(P))+"  ":GOSUB1950:PRINT
650 S$="FO:"+CAR$(3)+" > ":GOSUB1900:GOSUB19000:GOSUB 20010:FO(P)=D1+D2+15+BC(CP(P),3)
655 S$=STR$(BC(CP(P),3))+" >":PRINT@26,13;S$;:S$=STR$(FO(P))+"  ":GOSUB1950:PRINT
670 S$="AG:"+CAR$(4)+" > ":GOSUB1900:GOSUB19000:GOSUB 20010:AG(P)=D1+D2+15+BC(CP(P),4)
675 S$=STR$(BC(CP(P),4))+" >":PRINT@26,16;S$;:S$=STR$(AG(P))+"  ":GOSUB1950:PRINT
680 S$="QI:"+CAR$(5)+" > ":GOSUB1900:GOSUB19000:GOSUB 20010:IN(P)=D1+D2+15+BC(CP(P),5)
685 S$=STR$(BC(CP(P),5))+" >":PRINT@26,19;S$;:S$=STR$(IN(P))+"  ":GOSUB1950:PRINT
690 S$="FM:"+CAR$(6)+" > ":GOSUB1900:GOSUB19000:GOSUB 20010:FM(P)=D1+D2+15+BC(CP(P),6)
...
1525 S$=M$(MP(P)):IF MP(P)=1THENS$="Roturier"
...
1810 PRINT "Vous allez creer 6 personnages, choi-"
1812 PRINT "sissez leur un prenom et une carriere"
1814 PRINT "ensuite choisissez leur une maison...":PRINT
1820 PRINT "Comme dans tous les jeux de roles, il"
1822 PRINT "faut lancer 2 D10 pour definir les  6"
1824 PRINT "caracteristiques et les Points de vie":PRINT
1830 PRINT "Equilibrez bien votre equipe, chaque "
1832 PRINT "choix a ses points forts et faibles  ":PRINT
1834 PRINT "Vous avez 3 essais par personnage... ":PRINT
1840 PRINT @6,18;"Bonne creation d'equipe ! ;-)":PRINT@15 ,22;CHR$(148)"< Espace > "CHR$(144)
...
2030 S$="   SAUVEGARDES EN COURS - PATIENCE  ":GOSUB 1950
...
50010 DATA "Aucune","KING'S LANDING", MARTELL,DORNE, BARATHEON,"STORM'S END"
50020 DATA TYRELL,HIGHGARDEN, GREYJOY,PIKE, ARRYN,EYRIE
50030 DATA LANNISTER,CASTERLY ROC, TULLY,RIVERRUN, STARK,WINTERFELL
50040 DATA Capacite de Combat, Capacite de Tir, Force
50045 DATA Agilite, Intelligence, Force Mentale

50049 REM  CC CT FO AGI IN FM
50050 DATA  9, 0, 9, 2, 2, 4: REM CHEVALIER
50060 DATA  7, 2, 6, 6, 6, 6: REM MERCENAIRE
50070 DATA  4, 8, 4,10, 0, 0: REM RANGER
50090 DATA  2, 2, 2, 6, 6, 8: REM SORCIER
50100 DATA  0, 0, 0, 4, 8, 6: REM MESTRE
50110 DATA -2,-2, 0, 4,10,10: REM SEPTON
